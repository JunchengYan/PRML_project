...
from sklearn.metrics import precision_recall_curve, auc,average_precision_score, precision_score, recall_score, classification_report, confusion_matrix
...
def _init_(args):
    ...
    os.system('cp refine.py logs' + '/' + args.exp_name + '/' + 'refine.py.backup')
    os.system('cp utils.py logs' + '/' + args.exp_name + '/' + 'utils.py.backup')

def evaluate(io, test_label, test_score, test_pred):
    ap = average_precision_score(test_label, test_score)
    io.cprint(f"mAP(AP): {ap:.4f}")
    

    # PRC曲线
    precision, recall, thresholds = precision_recall_curve(test_label, test_score)
    ...
    plt.xticks(np.arange(thresholds[0]-0.1, thresholds[-1]+0.1, step=0.1), rotation=30)
    plt.grid()
    plt.legend()
...
def test_and_eval(args, io, model, test_loader, device, eval=True, best_path=None):
    ...
    with torch.no_grad():
        for data, label in tqdm(test_loader):
            ...
            test_prob.append(y_pred[:, 1].detach().cpu().numpy()) # 保存预测为1类的概率
        ...
        #         print(i, test_true[i], test_pred[i])
...
def main(args, io, eval=True, train_set=None, valid_set=None, test_set=None, finetune=False):
    if args.model == 'svm':
        model = SVC(kernel='rbf')
    elif args.model == 'resnet':
        model = ResNet18(num_classes=2, use_dropout=False, use_init=False)
        if finetune:
            model.load_state_dict(torch.load('./logs/res_public/models/model.t7'))
    io.cprint(str(model))
    
    # 加载数据
    if train_set is None:
        io.cprint("Preparing public data...")
    ...
    # 训练及测试
    io.cprint("Start to train on public dataset...") 
    if isinstance(model, SVC):
        ...
        decision_score = model.decision_function(test_data) # SVM的决策边界
        if eval:
            evaluate(io, test_label, decision_score, test_pred)
        
        return model

    elif isinstance(model, ResNet18):
        model = train(args, io, model, train_loader, valid_loader, optimizer, scheduler, device, epochs)
        test_and_eval(args, io, model, test_loader, device, eval=eval)
        
        return model

def tsinghua_main(args, io, eval=True, train_set=None, valid_set=None, test_set=None, test_set_for_main=None):
    io.cprint("\nGet Model...")
    if args.model == 'svm':
        model = main(args, io, eval=False, train_set=train_set, valid_set=valid_set, test_set=test_set_for_main)
    elif args.model == 'resnet':
        model = ResNet18(num_classes=2, use_dropout=False, use_init=False)
    ...
    if isinstance(model, SVC):
        ...
        if eval:
            test_score = model.decision_function(test_data) # SVM在测试集上的决策分数
            evaluate(io, test_label, test_score, test_pred)
    
    elif isinstance(model, ResNet18):
        test_loader = DataLoader(test_set, batch_size=1, num_workers=16, shuffle=False, drop_last=False)
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        test_and_eval(args, io, model, test_loader, device, eval=eval, best_path='./logs/res_public/models/model.t7')
