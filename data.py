...
def load_scene_Tsinghua_data(data_dir):
    ...
    with open(f'{FILE_PREFIX}{1}_test.json', 'wt') as f:
        json.dump(data_set1, f, indent=4)
    with open(f'{FILE_PREFIX}{2}_test.json', 'wt') as f:
        json.dump(data_set2, f, indent=4) 
    with open(f'{FILE_PREFIX}{3}_test.json', 'wt') as f:
        json.dump(data_set3, f, indent=4)
    with open(f'{FILE_PREFIX}{4}_test.json', 'wt') as f:
        json.dump(data_set4, f, indent=4)
    with open(f'{FILE_PREFIX}{5}_test.json', 'wt') as f:
        json.dump(data_set5, f, indent=4)
    return data_set1, data_set2, data_set3, data_set4, data_set5

def load_Tsinghua_data(data_dir):
    DATA_DIR = data_dir
    FILE_PREFIX = 'tsinghua_full' 
    data_set = []
    for img_dir in Path(DATA_DIR).rglob('*.jpg'):
        img_dir = str(img_dir)
        label = 0 if 'no_stairs' in img_dir else 1
...
if __name__ == '__main__':
    ...
    # img = Image.open('./stair/public/stairs/down/IMG_20190415_140429.jpg')
    # img.save('star.png', 'PNG', quality=95)
    dataset = load_Tsinghua_data('./stair/tsinghua/')
    data_transform = transforms.Compose([
        transforms.Resize([256, 256]),
        transforms.ToTensor(),
    ])
    # img = Image.open('./stair/tsinghua/scene3/no_stairs/frame0129.jpg')
    # img.save('star.png', 'PNG', quality=95)
